...
const CATEGORY_COLORS: Record<string, string> = {
  Insurance: '#6d9197', 'Finding Care': '#4ade80', 'Mental Health': '#60a5fa', Medications: '#fbbf24', Pregnancy: '#f472b6',
}
    ...
    <AppShell>
      ...
      <section style={{ minHeight: '80dvh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', padding: '100px 24px 60px', position: 'relative', overflow: 'hidden' }}>
        ...
        <h1 style={{ fontSize: 'clamp(36px, 6vw, 72px)', fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.08, marginBottom: '24px', maxWidth: '820px' }}>
          {TITLE.map((w, i) => (
            <span key={i} style={{ display: 'inline-block', marginRight: '0.25em', opacity: words[i] ? 1 : 0, transform: words[i] ? 'translateY(0)' : 'translateY(18px)', transition: 'opacity 0.55s cubic-bezier(0.16,1,0.3,1), transform 0.55s cubic-bezier(0.16,1,0.3,1)', color: w === 'alone' ? '#6d9197' : 'inherit' }}>{w}</span>
          ))}
        </h1>
        ...
        <button
          ...
          style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 28px', borderRadius: '100px', background: '#6d9197', color: '#07070F', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: 700, fontFamily: 'inherit', transition: 'transform 0.25s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.25s', boxShadow: '0 4px 20px rgba(109,145,151,0.3)' }}
          onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 32px rgba(109,145,151,0.45)' }}
            ...
            {STORIES.map((s, i) => (
              <RevealBlock key={s.name} delay={i * 100}>
                <div style={{ padding: '2px', background: 'linear-gradient(135deg, rgba(109,145,151,0.2), rgba(109,145,151,0.04))', borderRadius: '20px' }}>
                  <div style={{ background: '#0d1618', borderRadius: '18px', padding: '28px', borderLeft: '3px solid rgba(109,145,151,0.4)' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '28px' }}>
              {CATEGORIES.map(cat => (
                <button
                  ...
                  style={{ padding: '7px 16px', borderRadius: '100px', border: '1px solid', borderColor: activeCategory === cat ? 'rgba(109,145,151,0.4)' : 'rgba(255,255,255,0.1)', background: activeCategory === cat ? 'rgba(109,145,151,0.12)' : 'transparent', color: activeCategory === cat ? '#6d9197' : 'rgba(255,255,255,0.5)', fontSize: '12px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}
                >
                  {cat}
                </button>
              ))}
            ...
            {filteredPosts.map((p, i) => (
              <RevealBlock key={p.title} delay={i * 60}>
                <div style={{ padding: '20px 24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', transition: 'border-color 0.25s, background 0.25s' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(109,145,151,0.2)'; (e.currentTarget as HTMLElement).style.background = 'rgba(109,145,151,0.03)' }}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '10px', padding: '3px 9px', borderRadius: '100px', background: `${CATEGORY_COLORS[p.category] || '#6d9197'}15`, color: CATEGORY_COLORS[p.category] || '#6d9197', border: `1px solid ${CATEGORY_COLORS[p.category] || '#6d9197'}25`, fontWeight: 500 }}>{p.category}</span>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px', flexShrink: 0 }}>
                      <button
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '6px 12px', borderRadius: '100px', background: upvotes[i] ? 'rgba(109,145,151,0.12)' : 'rgba(255,255,255,0.04)', border: `1px solid ${upvotes[i] ? 'rgba(109,145,151,0.3)' : 'rgba(255,255,255,0.08)'}`, color: upvotes[i] ? '#6d9197' : 'rgba(255,255,255,0.4)', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}
                      >
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  ...
                  <textarea value={form.story} onChange={e => setForm(p => ({ ...p, story: e.target.value.slice(0, 2000) }))}
                    ...
                    rows={6}
                    style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '9px', padding: '11px 14px', color: '#eef4f5', fontSize: '14px', fontFamily: 'inherit', resize: 'vertical', outline: 'none', boxSizing: 'border-box', caretColor: '#6d9197', minHeight: '140px', transition: 'border-color 0.2s, box-shadow 0.2s' }}
                    onFocus={e => { e.currentTarget.style.borderColor = 'rgba(109,145,151,0.45)'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(109,145,151,0.08)' }}
                    onBlur={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'; e.currentTarget.style.boxShadow = 'none' }}
                  />
                ...
                <label style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', cursor: 'pointer' }}>
                  <input type="checkbox" checked={form.consent} onChange={e => setForm(p => ({ ...p, consent: e.target.checked }))} style={{ marginTop: '2px', width: '16px', height: '16px', accentColor: '#6d9197', flexShrink: 0 }} />
            ...
            {[
              ...
              <RevealBlock key={r.title} delay={i * 80}>
                <div style={{ padding: '2px', background: 'linear-gradient(135deg, rgba(109,145,151,0.18), rgba(109,145,151,0.04))', borderRadius: '18px' }}>
                  <div style={{ background: '#0d1618', borderRadius: '16px', padding: '24px', height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'rgba(109,145,151,0.1)', border: '1px solid rgba(109,145,151,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6d9197' }}>{r.icon}</div>
                    ...
                    <button onClick={r.action} style={{ padding: '10px 18px', borderRadius: '100px', background: 'rgba(109,145,151,0.1)', border: '1px solid rgba(109,145,151,0.2)', color: '#6d9197', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '6px', transition: 'background 0.2s', alignSelf: 'flex-start' }}
