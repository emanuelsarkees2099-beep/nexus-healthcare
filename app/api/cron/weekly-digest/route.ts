...
async function sendDigestEmail(stats: WeeklyStats): Promise<boolean> {
  ...
  const digestEmail = process.env.DIGEST_EMAIL || 'team@axvo.health'
  ...
  const html = `
    ...
    <body>
      ...
      <div class="footer">
        Generated by AXVO Impact Bot · <a href="https://axvo.health" style="color:#6d9197">axvo.health</a><br>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'AXVO Impact <digest@axvo.health>',
        to: [digestEmail],
