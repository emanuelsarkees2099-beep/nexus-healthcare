...
async function sendDigestEmail(stats: WeeklyStats): Promise<boolean> {
  ...
  const digestEmail = process.env.DIGEST_EMAIL || 'team@axvohealth.com'
  ...
  const html = `
    ...
    <body>
      <h1>AXVO Weekly Impact Digest</h1>
      ...
      <div class="footer">
        Generated by AXVO Impact Bot · <a href="https://www.axvohealth.com" style="color:#6d9197">axvohealth.com</a><br>
        This email is sent to the AXVO team only. No patient data is included.
      </div>
    </body>
    </html>
  `
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'AXVO Impact <digest@axvohealth.com>',
        to: [digestEmail],
        subject: `AXVO Weekly Impact — ${weekLabel}`,
        html,
      }),
    })
    return res.ok
